...
public class NoteTextIndex {
	...
	public void buildIndex(IProgressMonitor monitor) throws IOException{
		...
		indexWriter = new IndexWriter(fsDir,  
				new IndexWriterConfig(Version.LUCENE_CURRENT,new ClassicAnalyzer(Version.LUCENE_CURRENT)));
		indexNotes(monitor);
	...
	private void addToIndex(int key, String title, String content) {
	    ...
	    doc.add(new Field("key",keyString,Field.Store.YES,Field.Index.NO));
	...
	public Vector search(String queryString) throws ParseException{
		...
		QueryParser qp = new QueryParser(Version.LUCENE_CURRENT,"text",stdAn);
